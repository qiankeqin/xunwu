...
function load(city, regions, aggData) {
    ...
    //var point = new BMap.Point(116.403694,39.915156);
    var point = new BMap.Point(city.baiduMapLongitude, city.baiduMapLatitude); // 城市中心,可以使用百度地图坐标获取地图的经纬度
...
function drawRegion(map, regionList) {
    ...
    for (var i = 0; i < regionList.length; i++) {
        ...
        // 记录行政区域覆盖物
        polygonContext[textContent] = []; // 点集（多边形）
        ...
        textLabel.addEventListener('click', function (event) {
            ...
            map.zoomIn();//放大地图
            map.panTo(event.point);//鼠标进入到时间到中心
        });
    }
